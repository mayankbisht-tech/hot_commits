...
import { 
  ...
  Trash2, Building2, Calendar as CalendarIcon, CheckCircle2, AlertCircle, Users, Download, Loader2, Sparkles, MapPin, Check, Edit3
} from 'lucide-react';
...
export default function DriveConfiguration() {
  ...
  const [ctc, setCtc] = useState('14.5');
  const [location, setLocation] = useState('New Delhi / NCR');
  const [mode, setMode] = useState('HYBRID');
  const [driveDateStr, setDriveDateStr] = useState('2026-09-15');

  const [minCgpa, setMinCgpa] = useState<number>(7.0);
  const [maxBacklogs, setMaxBacklogs] = useState<number>(0);
  const [class12, setClass12] = useState<number>(60);
  const [class10, setClass10] = useState<number>(60);
  
  const { data: yearsData } = useSWR<{ years: string[] }>('/api/grad-years', fetcher);
  const availableYears = yearsData?.years || ['2027', '2028', '2029', '2030'];
  ...
  const handleAddBranch = async (e: React.FormEvent) => {
    e.preventDefault();
    const b = newBranchInput.trim().toUpperCase();
    if (!b) return;
    try {
  ...
  const handleDeleteBranch = async (branchToDelete: string) => {
    try {
      await fetch(`/api/branches?name=${encodeURIComponent(branchToDelete)}`, {
        method: 'DELETE',
        credentials: 'include'
      });
      mutate('/api/branches');
  ...
  const handleSaveConfig = async () => {
    ...
    if (!targetComp) {
      showPopup('error', 'Select Company', 'Please select a target company first.');
      return;
    }
    ...
    scheduledDriveDate.setHours(0, 0, 0, 0);

    if (scheduledDriveDate < today) {
      showPopup('error', 'Invalid Drive Date', 'It is not possible to post or schedule a drive before the current date (in the past). Please select today or a future date.');
      return;
    }
    ...
    try {
      ...
      await apiCreateDrive({
        ...
        description: `Recruitment drive for ${targetComp.name} (${roleTitle})`,
      ...
      showPopup('success', 'Drive Scheduled', `Drive for ${targetComp.name} is now published on the calendar.`);
    } catch (err: any) {
  ...
  const openEditModal = (drive: DriveItem) => {
    ...
    setDriveDateStr(drive.driveDate ? drive.driveDate.split('T')[0] : '2026-09-15');
    setSelectedBranches(drive.branches || ['AI-DS', 'AI-ML', 'AR', 'IIOT']);
  ...
  const handleApproveDrive = async (driveId: string) => {
    ...
    try {
      ...
      await mutate('/api/reports/stats');
      await mutate('/api/notifications');
      showPopup('success', 'Drive Approved', 'Company drive approved and marked ACTIVE on the calendar!');
    } catch (e: any) {
  ...
  return (
    <div className="flex h-full w-full bg-[#F8F5EC] overflow-hidden text-[#1C1A1A] animate-fade-in select-none">
      {/* Left Panel - Drive Configuration */}
      <div className="w-[390px] bg-white border-r border-[#E3D8C4] overflow-y-auto shrink-0 flex flex-col h-full shadow-xs z-10 select-none">
        <div className="p-5 border-b border-[#E3D8C4] sticky top-0 bg-white z-20 flex items-center justify-between">
          <div>
            <h1 className="text-base font-bold text-[#1C1A1A] select-none">Configure Placement Drive</h1>
            <p className="text-xs text-[#5E544A] mt-0.5 font-medium">Scheduling & eligibility gating</p>
          </div>
        </div>

        <div className="p-5 space-y-5 flex-1 text-xs">
          {/* Card 1: Target Company */}
          <div className="bg-white border border-[#E3D8C4] rounded-2xl p-4 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#1C1A1A] uppercase tracking-wider text-[11px]">1. Target Company *</span>
              {selectedCompany && (
                <button 
                  onClick={() => setSelectedCompany(null)}
                  className="text-[#8B7B6F] hover:text-[#C85555] text-[11px] flex items-center gap-1 font-bold"
                >
                  Change
                </button>
              )}
            ...
            {!selectedCompany ? (
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8B7B6F]" />
                <input
                  type="text"
                  placeholder="Search registered recruiting companies..."
                  className="w-full pl-8 pr-3 py-2 border border-[#E3D8C4] rounded-xl text-xs outline-none focus:border-[#8B1A1A] focus:ring-1 focus:ring-[#8B1A1A] bg-[#F8F5EC] text-[#1C1A1A] placeholder-[#8B7B6F] select-text font-semibold"
                  value={searchCompany}
                  onChange={(e) => {
                ...
                />
                {showDropdown && (
                  <div className="absolute z-30 mt-1 w-full bg-white border border-[#E3D8C4] rounded-xl shadow-lg max-h-48 overflow-y-auto divide-y divide-[#E3D8C4]">
                    {filteredCompanies.length > 0 ? (
                      filteredCompanies.map(company => (
                        <button
                          key={company.id}
                          type="button"
                          className="w-full text-left px-3.5 py-2 hover:bg-[#F1E9D8] flex items-center justify-between text-xs"
                          onClick={() => {
                        ...
                        >
                          <span className="font-bold text-[#1C1A1A]">{company.name}</span>
                          <span className="text-[10px] bg-[#F8F5EC] text-[#5E544A] border border-[#E3D8C4] px-1.5 py-0.5 rounded font-bold uppercase">
                            {company.tier || 'Tier-2'}
                          </span>
                        </button>
                      ))
                    ) : (
                      <div className="p-3 text-[#8B7B6F] text-center text-xs">No matching companies found</div>
                    )}
                  </div>
                )}
              </div>
            ) : (
              <div className="bg-[#F1E9D8] border border-[#E3D8C4] rounded-xl p-3 flex items-center justify-between">
                <div>
                  <p className="font-bold text-[#1C1A1A] text-xs">{selectedCompany.name}</p>
                  <p className="text-[10px] text-[#5E544A]">{selectedCompany.industry || 'Technology'} • {selectedCompany.tier || 'Tier-1'}</p>
                </div>
                <button 
                  onClick={() => setSelectedCompany(null)}
                  className="p-1 text-[#8B7B6F] hover:text-[#1C1A1A] rounded-md"
                >
                  <X size={14} />
                </button>
              </div>
          ...
          {/* Role Details */}
          <div className="bg-white border border-[#E3D8C4] rounded-2xl p-4 shadow-xs space-y-3">
            <span className="font-bold text-[#1C1A1A] uppercase tracking-wider text-[11px]">2. Role & Remuneration</span>
            <div className="space-y-2">
              <div>
                <label className="block text-[11px] font-bold text-[#5E544A] mb-1">Role Title</label>
                <input
                  type="text"
                  value={roleTitle}
                  onChange={e => setRoleTitle(e.target.value)}
                  className="w-full border border-[#E3D8C4] rounded-xl px-3 py-1.5 text-xs bg-[#F8F5EC] text-[#1C1A1A] font-semibold select-text"
                  placeholder="e.g. SDE-1 / Data Analyst"
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-bold text-[#5E544A] mb-1">CTC Package (LPA)</label>
                  <input
                    type="number"
                    step="0.1"
                    value={ctc}
                    onChange={e => setCtc(e.target.value)}
                    className="w-full border border-[#E3D8C4] rounded-xl px-3 py-1.5 text-xs bg-[#F8F5EC] font-bold text-[#8B1A1A] select-text"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-[#5E544A] mb-1">Drive Date</label>
                  <input
                    type="date"
                    value={driveDateStr}
                    onChange={e => setDriveDateStr(e.target.value)}
                    className="w-full border border-[#E3D8C4] rounded-xl px-3 py-1.5 text-xs bg-[#F8F5EC] text-[#1C1A1A] font-semibold"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-[#E3D8C4] rounded-2xl p-4 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#1C1A1A] uppercase tracking-wider text-[11px]">3. Academic Gating</span>
            </div>

            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <span className="font-bold text-[#5E544A]">Minimum CGPA</span>
                <span className="text-[#8B1A1A] font-extrabold bg-[#F1E9D8] border border-[#E3D8C4] px-2 py-0.5 rounded text-xs">
              ...
              <input 
                ...
                className="w-full accent-[#8B1A1A] h-1.5 bg-[#F1E9D8] rounded-lg cursor-pointer" 
              />
            </div>

            <div className="space-y-1.5">
              <label className="block font-bold text-[#5E544A]">Max Active Backlogs Allowed</label>
              <input 
                type="number"
                min={0}
                step={1}
                ...
                className="w-full border border-[#E3D8C4] rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:border-[#8B1A1A] focus:ring-1 focus:ring-[#8B1A1A] bg-[#F8F5EC] text-[#1C1A1A] select-text"
                placeholder="0"
              />
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E3D8C4] rounded-2xl p-4 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#1C1A1A] uppercase tracking-wider text-[11px]">4. Eligible Branches</span>
              <button 
                type="button"
                onClick={() => setShowAddBranch(!showAddBranch)}
                className="text-[11px] font-bold text-[#8B1A1A] hover:text-[#A63030] flex items-center gap-1"
              >
            ...
            {showAddBranch && (
              <form onSubmit={handleAddBranch} className="flex gap-2 p-2 bg-[#F1E9D8] border border-[#E3D8C4] rounded-xl animate-fade-in">
                <input 
                  type="text"
                  placeholder="e.g. AI-DS, IIOT"
                  value={newBranchInput}
                  onChange={(e) => setNewBranchInput(e.target.value)}
                  className="flex-1 px-2.5 py-1 text-xs border border-[#E3D8C4] rounded-lg bg-white text-[#1C1A1A] outline-none uppercase font-semibold select-text"
                  autoFocus
                />
                <button 
                  type="submit"
                  className="px-3 py-1 bg-[#8B1A1A] text-white rounded-lg text-xs font-bold hover:bg-[#A63030]"
                >
                  Add
                </button>
              </form>
                  ...
                  <div 
                    ...
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl border text-xs font-bold transition-all ${
                      isSelected 
                        ...
                        : 'bg-white text-[#5E544A] border-[#E3D8C4] hover:bg-[#F8F5EC]'
                    }`}
                  >
                    <button 
                      type="button" 
                    ...
                    <button 
                      ...
                      className={`p-0.5 rounded-full hover:bg-black/10 transition-colors ${
                        isSelected ? 'text-white/80 hover:text-white' : 'text-[#8B7B6F] hover:text-[#C85555]'
                      }`}
                      title={`Delete ${branch}`}
                    >
                      <X size={11} />
          ...
          <div className="pt-2">
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#8B1A1A] py-2.5 text-xs font-bold text-white hover:bg-[#A63030] transition-all shadow-xs active:scale-[0.98] disabled:opacity-50"
            >
              {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
              Save & Publish Drive on Calendar
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 flex flex-col h-full overflow-hidden select-none">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 bg-white p-4 rounded-2xl border border-[#E3D8C4] shadow-xs select-none">
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-[#F1E9D8] p-1 rounded-xl border border-[#E3D8C4]">
              <button 
                onClick={() => setViewMode('month')}
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  viewMode === 'month' ? 'bg-white text-[#8B1A1A] shadow-xs' : 'text-[#5E544A]'
                }`}
              >
                Month
              </button>
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  viewMode === 'week' ? 'bg-white text-[#8B1A1A] shadow-xs' : 'text-[#5E544A]'
                }`}
              >
                Week
              </button>
              <button 
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  viewMode === 'day' ? 'bg-white text-[#8B1A1A] shadow-xs' : 'text-[#5E544A]'
                }`}
              >
                Day
              </button>
            ...
            <button 
              onClick={handleToday}
              className="px-3 py-1 bg-white border border-[#E3D8C4] rounded-xl text-xs font-bold text-[#1C1A1A] hover:bg-[#F8F5EC] transition-colors"
            >
              Today
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-1.5 hover:bg-[#F1E9D8] rounded-xl text-[#5E544A] transition-colors border border-[#E3D8C4]"
              title="Previous Month"
            >
              <ChevronLeft size={16} />
            </button>
            <h2 className="text-base font-bold text-[#1C1A1A] min-w-36 text-center select-none">
              {monthName} {year}
            </h2>
            <button 
          ...
          <div className="text-xs text-[#5E544A] font-bold flex items-center gap-2">
        ...
        {/* Calendar Grid */}
        <div className="flex-1 bg-white rounded-2xl border border-[#E3D8C4] shadow-card flex flex-col overflow-hidden select-none">
          <div className="grid grid-cols-7 border-b border-[#E3D8C4] bg-[#F8F5EC] select-none">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} className="py-2.5 text-center text-[11px] font-bold text-[#5E544A] uppercase tracking-wider select-none">
                {day}
              </div>
            ))}
          </div>
          
          <div className="flex-1 grid grid-cols-7 grid-rows-5 bg-[#E3D8C4] gap-px overflow-y-auto select-none">
              ...
              return (
                <div 
                  ...
                  className={`bg-white p-2 flex flex-col justify-between transition-all min-h-[90px] select-none ${
                    isCurrentMonth ? 'cursor-pointer hover:bg-[#F8F5EC] group' : 'bg-[#F8F5EC]/50 text-[#8B7B6F]'
                  } ${selectedDay === dayNum ? 'ring-2 ring-[#8B1A1A] ring-inset bg-[#F1E9D8]/40' : ''}`}
                >
                  <div className="flex justify-between items-start select-none">
                    <span className={`text-xs font-bold h-6 w-6 flex items-center justify-center rounded-lg select-none ${
                      isToday 
                        ...
                        : isCurrentMonth ? 'text-[#1C1A1A] group-hover:text-[#8B1A1A]' : 'text-[#8B7B6F]'
                    }`}>
                      {isCurrentMonth ? dayNum : ''}
                    </span>
                    {dayDrives.length > 0 && (
                      <span className="text-[10px] font-bold bg-[#F1E9D8] text-[#8B1A1A] border border-[#E3D8C4] px-1.5 py-0.2 rounded select-none">
                        {dayDrives.length}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="space-y-1 mt-1 overflow-hidden select-none">
                    {dayDrives.map((d) => {
                      ...
                      return (
                        <div 
                          key={d.id} 
                          className={`text-[10px] px-1.5 py-0.5 rounded-md font-bold truncate transition-colors shadow-2xs select-none ${
                            isPending 
                              ? 'bg-[#F1E9D8] text-[#C8A243] border border-[#E3D8C4]' 
                              : 'bg-[#F1E9D8] text-[#8B1A1A] border border-[#E3D8C4] hover:bg-[#E3D8C4]'
                          }`}
                    ...
                    {isCurrentMonth && dayDrives.length === 0 && (
                      <span className="text-[10px] text-[#8B7B6F] opacity-0 group-hover:opacity-100 transition-opacity select-none">
                        + Schedule
                      </span>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-2xs p-4 overflow-y-auto animate-fade-in select-none">
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-[#E3D8C4] overflow-hidden animate-scale-in">
            <div className="p-4 bg-[#1C1A1A] text-white flex items-center justify-between">
              <div className="flex items-center gap-2">
                <CalendarIcon size={16} className="text-[#C8A243]" />
                <span className="font-bold text-xs">
                  Placement Schedule — {monthName} {selectedDay}, {year}
                </span>
              </div>
              <button onClick={() => setShowScheduleModal(false)} className="text-[#8B7B6F] hover:text-white">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4 text-xs">
              {dayDrivesForModal.length > 0 && (
                <div className="space-y-2">
                  <p className="font-bold text-[#5E544A] uppercase tracking-wider text-[10px]">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {dayDrivesForModal.map(d => {
                      const isPending = d.approvalStatus?.toLowerCase() === 'pending';
                      return (
                        <div key={d.id} className="p-3 bg-[#F8F5EC] rounded-xl border border-[#E3D8C4] flex justify-between items-center">
                          <div>
                            <div className="flex items-center gap-1.5">
                              <p className="font-bold text-[#1C1A1A]">{d.role}</p>
                              {isPending ? (
                                <span className="bg-[#F1E9D8] text-[#C8A243] border border-[#E3D8C4] text-[10px] font-bold px-1.5 py-0.2 rounded">
                                  Pending
                                </span>
                              ) : (
                                <span className="bg-[#F1E9D8] text-[#4A7C59] border border-[#E3D8C4] text-[10px] font-bold px-1.5 py-0.2 rounded">
                                  Approved ✓
                                </span>
                              )}
                            </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            {isPending && (
                              <button
                                ...
                                className="px-2.5 py-1 bg-[#4A7C59] hover:bg-[#3d6849] text-white rounded-lg font-bold text-[11px] flex items-center gap-1 shadow-xs"
                              >
                            ...
                            <button
                              ...
                              className="p-1 text-[#5E544A] hover:text-[#1C1A1A] hover:bg-[#F1E9D8] rounded-lg transition-colors"
                              title="Edit Drive"
                            >
                              <Edit3 size={13} />
                            </button>
              ...
              <div className="border-t border-[#E3D8C4] pt-3 space-y-2">
                <p className="font-bold text-[#5E544A] uppercase tracking-wider text-[10px]">
                  Schedule New Drive on {monthName} {selectedDay}
                </p>
                <div className="bg-[#F8F5EC] p-3 rounded-xl border border-[#E3D8C4] space-y-1 text-[11px]">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#8B1A1A] hover:bg-[#A63030] text-white rounded-xl font-bold flex items-center gap-1.5 shadow-xs"
                >
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-2xs p-4 overflow-y-auto animate-fade-in select-none">
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-[#E3D8C4] overflow-hidden animate-scale-in text-xs">
            ...
            <form onSubmit={handleUpdateDrive} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div>
                <label className="block font-bold text-[#5E544A] mb-1">Role Title</label>
                <input 
                  type="text"
                  required
                  value={roleTitle}

...
export default function DriveConfiguration() {
  ...
  const [driveDateStr, setDriveDateStr] = useState('2024-11-15');
  ...
  // REAL-TIME SAVE DRIVE TO DATABASE
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      showPopup('success', 'Drive Scheduled & Synced', `Drive for ${targetComp.name} is now published on the live calendar.`);
    } catch (err: any) {
  ...
  const openEditModal = (drive: DriveItem) => {
    ...
    setDriveDateStr(drive.driveDate ? drive.driveDate.split('T')[0] : '2024-11-15');
  ...
  return (
    <div className="flex h-full w-full bg-[#FFFAF6] overflow-hidden text-stone-800 animate-fade-in select-none">
      {/* Left Panel - Real-time Drive Configuration */}
      <div className="w-[390px] bg-white border-r border-stone-200 overflow-y-auto shrink-0 flex flex-col h-full shadow-xs z-10 select-none">
        <div className="p-5 border-b border-stone-100 sticky top-0 bg-white z-20 flex items-center justify-between">
          <div>
            <h1 className="text-base font-bold text-stone-900 select-none">Configure Placement Drive</h1>
            <p className="text-xs text-stone-500 mt-0.5">Real-time scheduling & eligibility gating</p>
          </div>
